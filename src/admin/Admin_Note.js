...
const NotesPage = ({ userData: initialUserData = { name: "" } }) => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await fetch("https://test-api-deploy-git-main-ntfs.vercel.app/api/note");
  ...
  const uploadImageChunks = async (imageData) => {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      const response = await fetch("https://test-api-deploy-git-main-ntfs.vercel.app/api/upload-chunk", {
        method: "POST",
  ...
  const handleCreateNote = async () => {
    try {
        ...
        for (let i = 0; i < totalImages; i++) {
            const file = newNote.images[i].file;
            if (!file) continue;
        ...
        alert(error.message || "Error creating note. Please try again.");
        setUploadProgress(0);
    }
};
  ...
  const handleEditNote = async () => {
    try {
      ...
      // Process each image
      for (let i = 0; i < totalImages; i++) {
        const imageData = newNote.images[i];
        let compressedImage;

        if (imageData.file) {
          // For new image files
          compressedImage = await compressImage(imageData.file);
          const imagePath = await uploadImageChunks(compressedImage.split(",")[1]);
          imagePaths.push(imagePath);
        } else if (imageData.url) {
          // For existing image URLs
          imagePaths.push(imageData.url.replace('https://test-api-deploy-git-main-ntfs.vercel.app', ''));
        }
        setUploadProgress(((i + 1) / totalImages) * 100);
      }
      ...
      const response = await fetch(`https://test-api-deploy-git-main-ntfs.vercel.app/api/note/${selectedNote.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(noteData),
      });
    ...
    try {
      ...
      setNewNote({
        ...
        images: result.images.map(image => ({
          ...
          url: `https://test-api-deploy-git-main-ntfs.vercel.app${image.Image_Path}`
        })),
        date: note.date,
      });
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(
        `https://test-api-deploy-git-main-ntfs.vercel.app/api/note/search?searchType=${searchType}&searchTerm=${encodeURIComponent(searchTerm)}`
      );
